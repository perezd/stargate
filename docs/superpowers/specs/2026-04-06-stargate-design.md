...
**Implementation:** The `ClassifyRequest` struct has a `DryRun bool` field tagged `json:"-"` (not accepted from HTTP JSON input). The `/test` handler sets `DryRun=true` before calling `Classify`. The classifier checks this field to skip corpus writes, cache reads/writes, and feedback token generation. This keeps the classification logic in a single code path.

**Optional field:** `use_cache` (bool, default false) — when true, allows cache reads during dry-run (for debugging caching behavior). Cache writes are always skipped regardless. Available in the `/test` request body and as `--cached` CLI flag.
...
- Is used by the `/feedback` endpoint to link user approval spans to the original trace

**`stargate_trace_id` is the OTel TraceID** — the hex-encoded 16-byte trace identifier from the classify span's trace context. It is generated by the OTel SDK via `trace.SpanFromContext(ctx).SpanContext().TraceID().String()`, not by stargate. This avoids a dual-identity problem and ensures traces are directly queryable in Grafana Tempo by `stargate_trace_id`.

**In-memory correlation map:** The `tool_use_id` from the agent is stored in a `ttlmap.TTLMap[string, string]` (`tool_use_id → stargate_trace_id`) with a 10-minute TTL and 10,000 entry cap. This allows PostToolUse feedback to join the correct trace without a filesystem read. On map miss, the adapter's trace file (M6) is the durable fallback.
...
| `stargate.scope.resolved` | string | Resolved scope value, truncated to 256 bytes (if resolver ran) |
...
- `stargate.llm.call.N` → Error on LLM timeout, API error, or malformed response
- `stargate.llm.file_read` → Error on denied path or read failure
...
**Trust assumption:** These overrides are intended for operator convenience (CI/CD secret injection). The process environment is trusted — an attacker with environment write access already has local code execution. A warning-level log is emitted at startup when any `STARGATE_OTEL_*` override is active, so operators can audit the effective configuration.
